...
function validReport(): string {
  return JSON.stringify({
    ...
    gitStatus: "",
    diffStat: "",
    validation: { commands: ["pnpm run validate"], configured: true },
    health: "good",
  });
}

describe("DefaultLoopCliReviewClient", () => {
  it("executes only the fixed review JSON command", async () => {
    ...
    const client = new DefaultLoopCliReviewClient(runner);
    const report = await client.loadProjectReview(
      "/tmp/loop-engine",
      "loop-engine",
    );
  ...
  it("rejects an empty repo path without invoking the runner", async () => {
    ...
    await assert.rejects(
      () => client.loadProjectReview(" ", "loop-engine"),
  ...
  it("rejects an empty project name without invoking the runner", async () => {
    ...
    await assert.rejects(
      () => client.loadProjectReview("/tmp/loop-engine", " "),
  ...
  it("rejects invalid JSON", async () => {
    ...
    await assert.rejects(
      () => client.loadProjectReview("/tmp/loop-engine", "loop-engine"),
      /invalid JSON/,
    );
  });
  ...
  it("rejects a non-zero CLI exit", async () => {
    ...
    await assert.rejects(
      () => client.loadProjectReview("/tmp/loop-engine", "missing"),
      /unknown project/,
    );
  });
});
