...
function validString(value: string, maximumBytes: number): boolean {
  return value.trim().length > 0 && byteLength(value) <= maximumBytes;
}

function validModel(value: string): boolean {
  return value.trim().length > 0 && value.length <= MAX_TEXT_ONLY_MODEL_CHARACTERS;
}
...
function validOutputTokenLimit(value: number): boolean {
  return (
    Number.isInteger(value) &&
    value > 0 &&
    value <= MAX_TEXT_ONLY_OUTPUT_TOKENS
  );
}
...
function parseUsage(value: unknown): TextOnlyProviderUsage | undefined {
  if (
    typeof value !== "object" ||
    value === null ||
    !Number.isInteger((value as AnthropicMessageResponse["usage"])?.input_tokens) ||
    !Number.isInteger((value as AnthropicMessageResponse["usage"])?.output_tokens)
  ) {
    return undefined;
  }
  return Object.freeze({
...
/**
 * Direct Anthropic Messages API adapter. It has no process, repository, tool,
 * filesystem, shell, Git, MCP, web, or code-execution integration.
 */
export function createAnthropicApiProvider(
  ...
  const maxOutputTokens = options.maxOutputTokens ?? MAX_TEXT_ONLY_OUTPUT_TOKENS;
  ...
  return Object.freeze({
    async invoke(input: TextOnlyProviderInput): Promise<TextOnlyProviderResult> {
      ...
      if (model === null) {
        return failure(null, "invalid_model", "Model is invalid.", now() - startedAt);
      }
