...
const SYSTEM = "Propose only a decision draft. You cannot authorize READY. Use only supplied JSON. Return JSON only. Propose minimal paths; never .git or the execution decision file. No tools, shell, web, Git, or filesystem.";
export class ExecutionDecisionProviderFailure extends Error { readonly failure: TextOnlyProviderFailure; constructor(failure: TextOnlyProviderFailure) { super("Execution decision provider failed."); this.failure = failure; } }
