...
export async function runExecutionDecisionProposal(input: ExecutionDecisionProposeInput, dependencies: Dependencies = {}): Promise<ExecutionDecisionCliResult> {
  ...
  try {
    ...
    if (!createExecutionDecisionDraft(current, completed.proposal).ok) return { schemaVersion: 1, project: input.project, result: { status: "failed", code: "decision_draft_invalid", model: completed.model, durationMs: completed.durationMs } };
