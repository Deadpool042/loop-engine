...
export async function runExecutionDecisionProposal(input: ExecutionDecisionProposeInput, dependencies: Dependencies = {}): Promise<ExecutionDecisionCliResult> {
  ...
  if (dependencies.createProvider === undefined && dependencies.propose === undefined) {
  ...
  try {
    ...
    if (!draft.draft.allowedPaths.includes(current.sourceDocument)) return { schemaVersion: 1, project: input.project, result: { status: "failed", code: "decision_draft_invalid", draftValidationIssue: "allowed_paths_invalid", ...telemetry } };
