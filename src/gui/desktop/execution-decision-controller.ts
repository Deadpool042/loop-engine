...
export function createDesktopExecutionDecisionController(options: Readonly<{ keychainReader: ProviderKeychainReader; cliInvoker: CliInvoker; resolveRepositoryPath: () => string | null; service?: Pick<ReturnType<typeof createProductionExecutionDecisionService>, "prepare" | "approve"> }>) {
  const service = options.service ?? createProductionExecutionDecisionService(createCliExecutionDecisionProposer(options));
