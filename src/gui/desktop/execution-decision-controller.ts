...
import { createCliExecutionDecisionProposer, ExecutionDecisionCliBoundaryFailure } from "./execution-decision-cli-proposer.js";
import type { CliInvoker } from "../cli-invoker.js";
import { ExecutionDecisionPreparationStageFailure } from "../../governance/execution-decision-errors.js";
import { createCliExecutionDecisionCurrent, ExecutionDecisionCurrentCliFailure } from "./execution-decision-cli-current.js";
...
const messages: Readonly<Record<string, string>> = {
  ...
  provider_authentication_failed: "Authentification Anthropic refusée.", provider_permission_denied: "La clé Anthropic n’a pas accès à cette ressource.", provider_rate_limited: "Limite Anthropic atteinte.", provider_timeout: "Anthropic n’a pas répondu dans le délai prévu.", provider_request_failed: "La requête envoyée à Anthropic a été refusée.", provider_billing_failed: "La facturation Anthropic ne permet pas cette requête.", provider_not_found: "La ressource Anthropic demandée est introuvable.", provider_request_too_large: "La requête envoyée à Anthropic est trop volumineuse.", provider_response_invalid: "La réponse Anthropic est invalide.", provider_refused: "Anthropic a refusé cette demande.", provider_output_truncated: "La réponse Anthropic a été tronquée.", provider_server_error: "Le service Anthropic est temporairement indisponible.", provider_unavailable: "Le service Anthropic est temporairement indisponible.", repository_unavailable: "Le dépôt Loop Engine n’est pas disponible.", cli_spawn_failed: "Le processus de proposition n’a pas pu être lancé.", cli_timeout: "Le processus de proposition a dépassé le délai prévu.", cli_response_invalid: "Loop Engine n’a pas pu interpréter la réponse du processus de proposition.", execution_decision_current_failed: "Loop Engine n’a pas pu lire l’état courant de la décision.", execution_decision_current_spawn_failed: "Le processus de lecture de la décision n’a pas pu être lancé.", execution_decision_current_timeout: "Le processus de lecture de la décision a dépassé le délai prévu.", execution_decision_current_response_invalid: "Loop Engine n’a pas pu interpréter l’état courant de la décision.", execution_decision_propose_failed: "Loop Engine a rencontré une erreur interne pendant la préparation de la décision.", execution_decision_draft_failed: "Loop Engine a rencontré une erreur interne pendant la préparation de la décision.", execution_decision_internal_error: "Loop Engine a rencontré une erreur interne pendant la préparation de la décision.",
};
...
export function createDesktopExecutionDecisionController(options: Readonly<{ keychainReader: ProviderKeychainReader; cliInvoker: CliInvoker; currentCliInvoker?: CliInvoker; resolveRepositoryPath: () => string | null; current?: (projectName: string) => Promise<ExecutionDecisionCurrent | null>; service?: Pick<ReturnType<typeof createProductionExecutionDecisionService>, "prepare" | "approve"> }>) {
  const current = options.current ?? createCliExecutionDecisionCurrent({ cliInvoker: options.currentCliInvoker ?? options.cliInvoker, resolveRepositoryPath: options.resolveRepositoryPath });
  const service = options.service ?? createProductionExecutionDecisionService(createCliExecutionDecisionProposer(options), { current });
  return Object.freeze({
    async prepare(projectName: unknown): Promise<DesktopExecutionDecisionResult> {
      ...
      catch (error) { if (error instanceof ExecutionDecisionProviderFailure) { const failure = error.failure; return failed(failure.code, undefined, { ...(failure.model === null ? {} : { model: failure.model }), durationMs: failure.durationMs, ...(failure.httpStatus === undefined ? {} : { httpStatus: failure.httpStatus }), failureCode: failure.code }); } if (error instanceof ExecutionDecisionPreparationStageFailure) { if (process.env.NODE_ENV !== "production") console.error(`[execution-decision] prepare failed stage=${error.stage} code=${error.code}`); return failed(error.code); } return failed(error instanceof ExecutionDecisionCliBoundaryFailure || error instanceof ExecutionDecisionCurrentCliFailure ? error.code : error instanceof Error && error.message === "keychain_unavailable" ? "keychain_unavailable" : error instanceof Error && error.message === "decision_draft_stale" ? "decision_draft_stale" : "execution_decision_internal_error", error instanceof Error && error.message === "keychain_unavailable" ? "Identifiant Anthropic indisponible dans le trousseau macOS." : undefined); }
    },
