...
const lotSchema = {
  ...
  properties: {
    title: stringSchema(MAX_ROADMAP_PROPOSAL_TITLE_CHARACTERS),
    objective: stringSchema(MAX_ROADMAP_PROPOSAL_TEXT_CHARACTERS),
    benefit: stringSchema(MAX_ROADMAP_PROPOSAL_TEXT_CHARACTERS),
...
function strings(
  ...
  return (
    ...
    value.every(
      (item) =>
        typeof item === "string" && item.length > 0 && item.length <= maxChars,
    )
  );
}
...
function parseProposal(
  output: string,
  projectName: string,
): { assessment: RoadmapProposalAssessment; proposal: RoadmapProposal } | null {
  if (bytes(output) > MAX_ROADMAP_PROPOSAL_BYTES) return null;
  let parsed: unknown;
  try {
  ...
  if (
    ...
    !isObject(parsed.assessment) ||
    !isObject(parsed.proposal)
  )
    return null;
  const { observedGaps, assumptions } = parsed.assessment;
  if (
    !strings(
      observedGaps,
    ...
    return null;
  if (parsed.proposal.status === "no_proposal") {
    if (
      typeof parsed.proposal.reason !== "string" ||
      parsed.proposal.reason.length === 0 ||
      parsed.proposal.reason.length > MAX_ROADMAP_PROPOSAL_TEXT_CHARACTERS ||
      "summary" in parsed.proposal ||
      "lots" in parsed.proposal
    )
      return null;
    return {
      assessment: { observedGaps, assumptions },
      proposal: { status: "no_proposal", reason: parsed.proposal.reason },
    };
  }
  if (
    parsed.proposal.status !== "proposed" ||
    typeof parsed.proposal.summary !== "string" ||
    parsed.proposal.summary.length === 0 ||
    parsed.proposal.summary.length > MAX_ROADMAP_PROPOSAL_TEXT_CHARACTERS ||
    !Array.isArray(parsed.proposal.lots) ||
    parsed.proposal.lots.length < 1 ||
    parsed.proposal.lots.length > MAX_ROADMAP_PROPOSAL_LOTS ||
    "reason" in parsed.proposal
  )
    return null;
  const lots = [] as {
    title: string;
  ...
  for (const lot of parsed.proposal.lots) {
    if (
      !isObject(lot) ||
      typeof lot.title !== "string" ||
      lot.title.length === 0 ||
      lot.title.length > MAX_ROADMAP_PROPOSAL_TITLE_CHARACTERS ||
      typeof lot.objective !== "string" ||
      lot.objective.length === 0 ||
      lot.objective.length > MAX_ROADMAP_PROPOSAL_TEXT_CHARACTERS ||
      typeof lot.benefit !== "string" ||
      lot.benefit.length === 0 ||
      lot.benefit.length > MAX_ROADMAP_PROPOSAL_TEXT_CHARACTERS ||
      !(["low", "medium", "high"] as const).includes(lot.cost as never) ||
      !(["low", "medium", "high"] as const).includes(lot.risk as never) ||
      !strings(
        lot.dependencies,
    ...
    lots.push({
      ...
      benefit: lot.benefit,
      cost: lot.cost as "low" | "medium" | "high",
      risk: lot.risk as "low" | "medium" | "high",
      dependencies: lot.dependencies,
    });
  }
  return {
    assessment: { observedGaps, assumptions },
    proposal: { status: "proposed", summary: parsed.proposal.summary, lots },
  };
}
export async function generateRoadmapProposalFromContext(
  ...
  const proposal = parseProposal(result.output, context.project.name);
  if (proposal === null)
    return {
      schemaVersion: 1,
      project: { name: context.project.name },
      result: { status: "failed", reason: "invalid_proposal_response" },
    };
  return {
    schemaVersion: 1,
    project: { name: context.project.name },
