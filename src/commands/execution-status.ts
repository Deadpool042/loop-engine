...
function expectedDurationMs(
  application: Pick<LoopApplicationAssembly, "generateRunHistoryReport">,
  project: string,
): number | null {
  const history = application.generateRunHistoryReport(project, {
    limit: HISTORY_LIMIT,
  });
  return median(
    history.entries
...
export async function buildExecutionStatusReport(
  ...
  const expectedMs =
    options.expectedDurationMs === undefined
      ? expectedDurationMs(application, project)
...
export async function printExecutionStatusJson(
  ...
  console.log(JSON.stringify(await buildExecutionStatusReport(application, project)));
}
