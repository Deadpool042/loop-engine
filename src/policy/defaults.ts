...
const MODE_PERMISSION_CEILINGS: Readonly<
  Record<AgentPolicyMode, readonly AgentPermission[]>
> = {
  plan: ["read_only"],
...
export function mergeBudgetsRestrictively(
  globalBudget: AgentBudget,
  requestedBudget: AgentBudget,
): AgentBudget {
  const result = {} as Record<
    (typeof BUDGET_DIMENSIONS)[number],
    number | null
  >;
...
export function restrictMaximumEffort(
  globalMax: AgentEffort,
  requestedMax: AgentEffort | undefined,
): AgentEffort {
  if (!requestedMax) {
    return globalMax;
  }

  return compareAgentEffort(requestedMax, globalMax) <= 0
    ? requestedMax
    : globalMax;
}
...
export function mergeContextBudgetsRestrictively(
  ...
  return {
    ...
    maxEstimatedTokens: Math.min(
      global.maxEstimatedTokens,
      requested.maxEstimatedTokens,
    ),
