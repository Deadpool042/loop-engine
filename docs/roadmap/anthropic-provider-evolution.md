...
| Structured outputs (`output_config.format`)                    | EXISTING             | schema wired; `strict: true` was set in R1 but corrected out 2026-08-24 (not a valid `output_config.format` field — see R1 correction below) |
...
- Decisions actually taken:
  ...
  - `strict: true` was added to `output_config.format` in `anthropic-api-provider.ts` unconditionally whenever an `outputSchema` is provided; the contract shape (`{ type: "json_schema", schema, strict }`) is additive and does not change any existing field. **Correction (2026-08-24):** this was factually wrong. A real Anthropic Messages API call confirmed `strict` is not a valid field of `output_config.format` (it returns HTTP 400 `provider_request_failed`) — `strict: true` is a top-level field on a *tool* definition ("strict tool use"), unrelated to structured outputs. `strict` was removed from `output_config.format`; the transmitted shape is `{ type: "json_schema", schema }` only. See the corrected capability-snapshot row above.
