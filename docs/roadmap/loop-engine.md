...
- [x] [P3] V48.5 — Escalade intra-provider bornée livrée : le LoopRunner conserve le chemin historique à un appel par défaut et n'autorise une seconde tentative que via `allowEscalation` + budget `maxCalls` effectif. La primitive V48.5 `selectIntraProviderModelEscalation` est séparée de l'escalade Agent historique V7/V13 : `validation_failed` peut monter vers le plus petit tier économique supérieur admissible, un `capability_gap` doit être démontré et peut sauter directement vers le tier qui le comble, tandis que `runtime_error` et `budget_exceeded` n'augmentent jamais le modèle. Provider/runtime, effort, scope, permissions et validations restent inchangés ; l'escalade est unique, revalidée et ne se cumule pas avec le failover multi-provider. Validation complète : 2635/2635 tests verts.
