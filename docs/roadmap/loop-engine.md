...
Contexte initial du 2026-09-05 : Codex CLI et Claude Code étaient intégrés comme providers concrets mais n'avaient encore aucune exécution réelle gouvernée dans le Run History. V48.0 a depuis qualifié le chemin réel avec Codex / GPT-5.6 Luna ; le run `20fc0188-ddf2-41b7-a8f4-161e5116cea4` a atteint le provider, puis `pnpm run validate` avec succès dans le worktree isolé après correction de la préparation des dépendances `production`. La conception détaillée, les gates et les améliorations futures sont dans [model-routing-v48.md](model-routing-v48.md).

- [x] [P1] V48.0 — Premier execute réel gouverné qualifié avec Codex / GPT-5.6 Luna : run `20fc0188-ddf2-41b7-a8f4-161e5116cea4`, provider terminé sur la première tentative, `pnpm run validate` passé, 0 réparation, dépôt source inchangé. Le burn-in a révélé puis corrigé la préparation absente des dépendances `workspace.dependencies: production` dans le worktree isolé ; la réhydratation pnpm est désormais offline, lockfile figé, scripts désactivés et fail-closed.
- [x] [P1] V48.1 — Burn-in multi-provider minimal qualifié : Claude Code / Haiku 4.5 a été exercé réellement avec résultats gouvernés `provider_timeout` puis `provider_max_turns` sans mutation du dépôt source ; le failover canonique exposé par `loop run` a ensuite été démontré sur le run `95e7560f-7e19-4515-995d-439ad2708b79`, avec tentative Anthropic `provider_unavailable` récupérable puis Codex / GPT-5.6 Luna `completed`, `pnpm run validate` exit 0, 0 réparation et 0 fichier modifié. Haiku n'est pas qualifié comme executor autonome suffisant pour ce candidat générique ; le failover inter-provider existant est qualifié et reste l'unique mécanisme.
- [x] [P2] V48.2 — Portefeuille de modèles configurable livré : un même provider/runtime peut exposer plusieurs profils de modèles sur un seul executable avec modèle libre, capacités explicites, `economicTier` ordonné (`economy|standard|advanced|frontier`) et `availability` explicite. Le mode historique à modèle unique reste compatible mais conservateur ; aucune capacité enrichie n'est désormais déduite d'un nom commercial. Les profils indisponibles sont rejetés par le sélecteur et ignorés par le failover. Les gammes Luna/Terra/Sol/Astra et Haiku/Sonnet/Opus/Fable sont représentables comme simple configuration révisable.
- [x] [P2] V48.3 — Sélection coût/capacité déterministe livrée : le selector unique applique d'abord les hard gates (capacités, permissions, allow-lists provider/runtime, disponibilité, effort maximum et budget), puis classe uniquement les profils admissibles par `economicTier` (`economy < standard < advanced < frontier`). Les profils sans tier restent compatibles mais passent après tout tier explicite, sans coût inventé ; à tier égal ou pour un registre legacy entièrement non classé, le tie-break historique `effort -> id` reste déterministe. L'evidence `notSelected` distingue tier plus élevé, tier inconnu, effort supérieur et simple tie-break.
- [x] [P2] V48.4 — Evidence d'efficacité par modèle livrée : `pnpm loop runs <project> --models [--json] [--limit N]` projette depuis le Run History existant provider, runtime, modèle, profil, effort lorsqu'il est attribuable, catégorie de tâche issue de la policy, statut terminal, failover/tentatives, raisons d'échec, validations, réparations, durée, fichiers modifiés et hors scope. Les agrégats restent bornés au journal lu et les runs non attribuables sont comptés explicitement. Tokens, coût et quota restent `unavailable` tant qu'aucune source provider fiable n'est enregistrée ; aucun journal analytics parallèle ni chiffre inventé n'est créé.
- [x] [P3] V48.5 — Escalade intra-provider bornée livrée : la tentative initiale peut être suivie d'au plus une montée de modèle, bornée par `AgentPolicy.allowEscalation` + `selectionRequest.budgetCeiling.maxCalls`. Seuls `provider_max_turns` et `validation_failed` après épuisement des réparations justifient cette montée ; timeout, rate-limit, indisponibilité et erreurs runtime restent au failover/stop. Le profil suivant reste sur le même provider/runtime, conserve scope/permissions/validations et doit satisfaire tous les hard gates. `modelEscalationEvidence` rend la transition observable.
- [x] [P3] V48.6 — Stratégie abonnement / crédits / API livrée : les profils peuvent déclarer un financement (`included_subscription`, crédits additionnels, API facturée ou `unknown`) et un snapshot quota borné (`available | exhausted | unknown`) sans estimation. Le selector refuse tout financement payant sans opt-in explicite, privilégie l'usage inclus à capacité comparable, laisse `unknown` non bloquant et rejette uniquement un quota explicitement épuisé. La policy est l'autorité d'autorisation payante ; les demandes caller ne peuvent que la restreindre. Failover et escalation réutilisent cette même admission, sans second router ni scraping de quota.
...
- [x] [P1] V51.1 — Burn-in réel du CTA `Continuer` réconcilié : run `802a5cc6-d6ca-4de9-be53-594faeb27546` terminé, validation complète passée après escalade Haiku → Sonnet, candidate ref locale `fa3dbb5e` revue et dépôt source resté propre ; le transport durable a ensuite été durci et mergé par #263. [Preuve](../audits/auto-continuation-burnin.md) · [Détail](auto-subscription-decision-renewal-v51.md)
...
Contexte vérifié le 2026-09-07 : `completion-events --json` couvre déjà `lot.completed` et `execution.failed`. Le burn-in OpenClaw a aussi qualifié la séparation provider/validation : Claude produit le delta borné, puis Loop Engine exécute la validation et la publication candidate. Le manque fonctionnel restant pour OC-10 est l’observabilité déterministe d’une gate canonique nécessitant une intervention utilisateur.
...
- aucune validation/CI exécutée par Claude : Loop Engine reste propriétaire de cette phase ;
- aucun appel IA supplémentaire hors runtime sélectionné ;
- `pnpm run ci` doit rester vert avant publication candidate.
