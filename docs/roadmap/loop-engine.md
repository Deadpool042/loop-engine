...
La source de décision reste l'audit `docs/audits/architecture-delivery-readiness-v14.2u.md`; les architectures V14.3 et V14.4 sont documentées dans `docs/architecture/prepared-inbound-runtime-execution.md` et `docs/architecture/looprunner-execute-validation-repair.md`.
...
- [ ] Lot V14.5 — Concrete Inbound Security and Adapter Pilot : identité, ACL et replay persistants injectés, puis un seul adapter inbound transport-neutral ; aucune exécution provider implicite

## Séquence suivante — bloquée par V14.5
...
- V14.5 doit livrer une seule verticale identité/ACL/replay/adapter, sans micro-lots par champ, getter, Promise ou token source.
