...
- [ ] Lot V23.1 — Recoverable isolated project locks : rendre les verrous d’exécution isolée récupérables après arrêt brutal sans permettre deux exécutions concurrentes du même projet. Le lock doit conserver une preuve minimale de propriétaire/ancienneté, distinguer un propriétaire encore vivant d’un lock manifestement orphelin, récupérer uniquement les locks périmés selon une règle déterministe et fail-closed en cas d’état ambigu. Couvrir au minimum : concurrence normale toujours refusée, lock abandonné après crash récupérable, lock actif jamais volé, métadonnée corrompue/illisible bloquée proprement, release idempotent et absence de suppression d’un lock remplacé par un autre propriétaire. Aucun parallélisme multi-projet, aucune queue, aucun terminal, aucun contrôle distant/mobile dans ce lot.
