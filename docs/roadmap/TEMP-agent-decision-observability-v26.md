...
- [x] V26.1a — Registry truth audit : le chemin de production exécutable dépend bien de profils illustratifs. `src/composition/provider-registry.ts` dérive `configured.codex` et `configured.claude_code` de `defaultAgentRegistry`, alors que `DEFAULT_AGENT_PROFILES` est explicitement documenté comme configuration illustrative. Les tests de provider registry figent aujourd'hui cette dérivation. Le gap est donc démontré et justifie un lot de correction séparé.
- [ ] V26.1b — Provider-bound registry truth : supprimer la dépendance de l'assemblage exécutable aux profils illustratifs sans dupliquer un catalogue de capacités. Réutiliser en priorité les contrats provider/capability déjà présents ; conserver `model` libre et l'admission fail-closed. Aucun changement du selector tant qu'il n'est pas nécessaire.
