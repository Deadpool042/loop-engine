...
RuntimeRequest is the immutable, versioned and serializable declarative request that follows a valid Execution Bridge. It records only caller-supplied identifiers, explicit timestamps, the validated Execution Bridge reference, and stable evidence references. It gives a future runtime layer a bounded contract to consider; it does not make that future layer available.
...
Execution Bridge remains the sole upstream evidence reference. RuntimeRequest validates its identifier, version, ready state, and its denied execution flags. It does not reinterpret governance, approval, eligibility, authority, transport, or provider policy. Future Runtime Resolution and a future Runtime Adapter, if ever introduced, MUST be specified by a separate RFC and MUST NOT be inferred from this contract.
...
The module reads no clock, environment, filesystem, network, process, registry, or mutable global state. All time is explicit input. Evidence references are normalized into stable lexical ordering.
