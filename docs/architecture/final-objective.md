...
Le comportement par défaut reste non destructif : pas d'appel IA automatique, pas de commit automatique, pas de push automatique, pas de modification des projets observés. Un `execute` explicitement configuré s'effectue dans un Git worktree isolé et temporaire, jamais dans le dépôt source. Ces garanties ne s'effacent jamais devant un mode explicitement sélectionné : pas de commit automatique et pas de push automatique restent la règle tant qu'un mode `commit` ou `publish` n'a pas été explicitement demandé par l'humain.
