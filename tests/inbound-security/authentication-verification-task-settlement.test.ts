...
describe("authentication verifier task settlement", () => {
  it("accepts valid evidence resolved from a later task", async () => {
    const result = {
  ...
  it("canonicalizes rejection delivered from a later task", async () => {
    const result = {
      then(_resolve: (value: unknown) => void, reject: (reason: unknown) => void) {
        setImmediate(() => reject(new Error("task rejection")));
      },
    };
